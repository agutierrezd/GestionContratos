...
function checkTableName($shortTName, $type=false)
{
	...
	if ("cargo" == $shortTName && ($type===false || ($type!==false && $type == 0)))
		return true;
	if ("q_informes_supervision_activos" == $shortTName && ($type===false || ($type!==false && $type == 0)))
		return true;
...
function GetTablesList($pdfMode = false)
{
	...
	$tableAvailable = true;
	if( $checkPermissions ) {
		$strPerm = GetUserPermissions("q_informes_supervision_activos");
		$tableAvailable = ( strpos($strPerm, "P") !== false
			|| $pdfMode && strpos($strPerm, "S") !== false );
	}
	if( $tableAvailable ) {
		$arr[]="q_informes_supervision_activos";
	}
	$tableAvailable = true;
	if( $checkPermissions ) {
...
//	default permissions
	...
	if( $table=="q_informes_supervision_activos" )
	{
//	default permissions
		return "ADESPI".$extraPerm;
	}
...
function SetAuthSessionData($pUsername, &$data, $password, &$pageObject = null, $fireEvents = true )
{
	...
	$_SESSION["_q_informes_supervision_activos_OwnerID"] = $data["Username"];
