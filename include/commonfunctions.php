...
function checkTableName($shortTName, $type=false)
{
	...
	if ("divipola" == $shortTName && ($type===false || ($type!==false && $type == 0)))
		return true;
	if ("contractor_master_vinc" == $shortTName && ($type===false || ($type!==false && $type == 1)))
		return true;
	if ("paa_master" == $shortTName && ($type===false || ($type!==false && $type == 0)))
		return true;
	if ("vigencia" == $shortTName && ($type===false || ($type!==false && $type == 0)))
		return true;
	if ("contrato_tipo" == $shortTName && ($type===false || ($type!==false && $type == 0)))
		return true;
	if ("q_rubros" == $shortTName && ($type===false || ($type!==false && $type == 0)))
		return true;
	if ("q_usos" == $shortTName && ($type===false || ($type!==false && $type == 0)))
		return true;
	if ("rep_prg001_catalogopresupuestal" == $shortTName && ($type===false || ($type!==false && $type == 0)))
		return true;
	if ("contrato_tipo_unidad" == $shortTName && ($type===false || ($type!==false && $type == 0)))
		return true;
	if ("interventor_periodos" == $shortTName && ($type===false || ($type!==false && $type == 0)))
		return true;
	if ("tparam_tipo_ct_000" == $shortTName && ($type===false || ($type!==false && $type == 0)))
		return true;
	if ("tparam_tipo_ct_001" == $shortTName && ($type===false || ($type!==false && $type == 0)))
		return true;
	if ("tparam_tipo_ct_002" == $shortTName && ($type===false || ($type!==false && $type == 0)))
		return true;
	if ("tipo_contrato" == $shortTName && ($type===false || ($type!==false && $type == 0)))
		return true;
	if ("contrato_forma_pago" == $shortTName && ($type===false || ($type!==false && $type == 0)))
		return true;
	if ("tipo_si_no" == $shortTName && ($type===false || ($type!==false && $type == 0)))
		return true;
	if ("contrato_estados" == $shortTName && ($type===false || ($type!==false && $type == 0)))
		return true;
	if ("contrato_pre" == $shortTName && ($type===false || ($type!==false && $type == 0)))
		return true;
	if ("contrato_attached" == $shortTName && ($type===false || ($type!==false && $type == 0)))
		return true;
	if ("contrato_attached_type" == $shortTName && ($type===false || ($type!==false && $type == 0)))
		return true;
	if ("polizas_master" == $shortTName && ($type===false || ($type!==false && $type == 0)))
		return true;
	if ("polizas_tipo" == $shortTName && ($type===false || ($type!==false && $type == 0)))
		return true;
	if ("polizas_aseguradoras" == $shortTName && ($type===false || ($type!==false && $type == 0)))
		return true;
	if ("info_secop" == $shortTName && ($type===false || ($type!==false && $type == 0)))
		return true;
	if ("interventor_interno" == $shortTName && ($type===false || ($type!==false && $type == 0)))
		return true;
	if ("tparam_habilita" == $shortTName && ($type===false || ($type!==false && $type == 0)))
		return true;
	if ("contrato_cdp" == $shortTName && ($type===false || ($type!==false && $type == 0)))
		return true;
	if ("dependencia" == $shortTName && ($type===false || ($type!==false && $type == 0)))
		return true;
	if ("contrato_rp" == $shortTName && ($type===false || ($type!==false && $type == 0)))
		return true;
	...
	if ("contrato_chart_supjer" == $shortTName && ($type===false || ($type!==false && $type == 3)))
		return true;
	if ("secop" == $shortTName && ($type===false || ($type!==false && $type == 0)))
		return true;
	if ("tparam_tipo_ct_rubro" == $shortTName && ($type===false || ($type!==false && $type == 0)))
		return true;
	...
	if ("tparam_sn_oe" == $shortTName && ($type===false || ($type!==false && $type == 0)))
		return true;
	if ("contrato_modifica" == $shortTName && ($type===false || ($type!==false && $type == 0)))
		return true;
	if ("contrato_modifica_type" == $shortTName && ($type===false || ($type!==false && $type == 0)))
		return true;
...
function GetTablesList($pdfMode = false)
{
	...
	$tableAvailable = true;
	if( $checkPermissions ) {
		$strPerm = GetUserPermissions("q_rubros");
		$tableAvailable = ( strpos($strPerm, "P") !== false
			|| $pdfMode && strpos($strPerm, "S") !== false );
	}
	if( $tableAvailable ) {
	...
	$tableAvailable = true;
	if( $checkPermissions ) {
		$strPerm = GetUserPermissions("q_usos");
		$tableAvailable = ( strpos($strPerm, "P") !== false
			|| $pdfMode && strpos($strPerm, "S") !== false );
	}
	if( $tableAvailable ) {
	...
	$tableAvailable = true;
	if( $checkPermissions ) {
		$strPerm = GetUserPermissions("secop");
		$tableAvailable = ( strpos($strPerm, "P") !== false
			|| $pdfMode && strpos($strPerm, "S") !== false );
	}
	if( $tableAvailable ) {
...
function ReadUserPermissions($userID = "")
{
	...
	if($userID != "Guest")
	{
		...
		$sql = "select ".$gConn->addFieldWrappers( "GroupID" )
			...
			." from ". $gConn->addTableWrappers( "gestioncontratos_ver_103_ugmembers" )
	...
	$sql = "select ". $gConn->addFieldWrappers( "TableName" )
		...
		." from ". $gConn->addTableWrappers( "gestioncontratos_ver_103_ugrights" )
